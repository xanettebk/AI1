...
drawWeather() {
    ...
    if (this.forecast && this.forecast.length > 0) {
        ...
        for (let i = 0; i < this.forecast.length; i++) {
            ...
            const dayString = date.toLocaleDateString("pl-PL");

            if (dayString !== currentDay) {
                // Create a new row container for each day
                rowContainer = document.createElement("div");
                rowContainer.className = "weather-row";
                this.resultsBlock.appendChild(rowContainer);
            ...
            const weatherBlock = this.createWeatherBlock(dateTimeString, temperature, feelsLikeTemperature, iconName, description);
            rowContainer.appendChild(weatherBlock);
        }
    }
}
